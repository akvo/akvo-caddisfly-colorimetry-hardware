...
print "Akvo logo, and should therefore be exluded from the calibration"
...
try:
    ...
    dataEndIndex = findIndex(lines,"END_DATA",exclude="BEGIN_DATA_FORMAT")
except ValueError:
...
for index in range(dataStartIndex + 1, dataEndIndex):
    ...
    if row[1] == "J2":
        print "skipping Akvo logo at location J2"
        continue
    result = "{\"l\":\"" + row[1] + "\",\"CIE_L\":\"" + row[LAB_L_index] + "\",\"CIE_A\":\"" + row[LAB_L_index + 1] + "\",\"CIE_B\":\"" + row[LAB_L_index + 2] + "\"" + "}"
