...
# The output is written to the /D65 folder. D65 is added to the name.
#
# author: M.T. Westra
# date: 3-8-2016
# version: 1.0
...
for inputFile in os.listdir(INPUTDIR):
    ...
    try:
        dataStartIndex = findIndex(lines, "BEGIN_DATA", exclude="BEGIN_DATA_FORMAT")
        dataEndIndex = findIndex(lines, "END_DATA", exclude="END_DATA_FORMAT")
        if dataStartIndex == -1 or dataEndIndex == -1:
    ...
    lines[14] = "AKVO - TRANSFERED TO D65 COLOUR SPACE\t" + \
                datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y") + "\n"
    lines[15] = "NUMBER_OF_FIELDS\t5\n"
    lines[17] = "SAMPLE_ID\tSAMPLE_NAME\tLAB_L\tLAB_A\tLAB_B\n"

    for index in range(dataStartIndex + 1, dataEndIndex):
        dInd = index - dataStartIndex - 1
        lines[index] = str(newData[dInd][0]) + "\t" + str(newData[dInd][1]) + "\t" + \
                    "%.3f" % newData[dInd][2] + "\t" + "%.3f" % newData[dInd][3] + "\t" + \
                    "%.3f" % newData[dInd][4] + "\n"
